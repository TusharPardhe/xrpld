...
`.to_nearest`.  Its scale is in the full IOU range `[-96, 80]`, and either the
result is nonzero or the scale is canonical (`s ≥ -81`).  Below `-81`, the
16-digit exponent clamp can flush a tiny positive grid coefficient to zero;
the explicit alternative rules out exactly that escape. -/
theorem STAmount.roundToExponent_rounded_proof (value result : STAmount) (s : ℤ)
    (mode : rounding_mode)
    (hc : value.IOUCanonical)
    (h_s : (-96 : ℤ) ≤ s) (h_s_hi : s ≤ 80)
    (h_survives : result.mValue ≠ 0 ∨ (-81 : ℤ) ≤ s)
    ...
    have h_res_val : result.toRat = (if value.mIsNegative then (-1 : ℚ) else 1)
      ...
      · -- k ≥ 1: the difference is an exactly representable grid point.
        ...
        have hexp_ge : cMinOffset ≤ w.exponent_ + 3 := by
          rcases h_survives with hnz | hs81
          · by_contra hlt
            push_neg at hlt
