...
/-- **Exponent order for positive normalized Numbers.** A positive normalized
`Number` has a 19-digit mantissa in `[10^18, 10^19)`. Consequently, if its
value is no larger than another positive normalized Number, its decimal
exponent is no larger either: a strictly larger source exponent would put its
whole canonical decade above the other value. -/
theorem Number.exponent_le_of_normalized_pos_toRat_le
    ...
    n₁.exponent_ ≤ n₂.exponent_ := by
  have hpos₂ : 0 < n₂.toRat := lt_of_lt_of_le hpos₁ horder
  have hmant₁ : n₁.mantissa_ ≠ 0 := by
    intro hz
    unfold Number.toRat at hpos₁
    rw [if_pos hz] at hpos₁
    norm_num at hpos₁
  have hmant₂ : n₂.mantissa_ ≠ 0 := by
    intro hz
    unfold Number.toRat at hpos₂
    rw [if_pos hz] at hpos₂
    norm_num at hpos₂
  obtain ⟨hmant₁_lo, _⟩ := hnorm₁.mantissaBounds_nat hmant₁
  obtain ⟨_, hmant₂_hi⟩ := hnorm₂.mantissaBounds_nat hmant₂
  ...
  have hneg₂ : n₂.negative_ = false := by
    ...
    | false => rfl
    | true => exact False.elim (by linarith [Number.toRat_nonpos_of_negative n₂ hs])
  have hmant₁_lo' : (10 : ℚ) ^ (18 : ℕ) ≤ (n₁.mantissa_.toNat : ℚ) := by
    have hcast : ((10 ^ 18 : ℕ) : ℚ) ≤ (n₁.mantissa_.toNat : ℚ) := by
      exact_mod_cast hmant₁_lo
    have hpow : ((10 ^ 18 : ℕ) : ℚ) = (10 : ℚ) ^ (18 : ℕ) := by
      push_cast
      ring
    rw [← hpow]
    exact hcast
  have hmant₂_hi' : (n₂.mantissa_.toNat : ℚ) < (10 : ℚ) ^ (19 : ℕ) := by
    have hcast : (n₂.mantissa_.toNat : ℚ) < ((10 ^ 19 : ℕ) : ℚ) := by
      exact_mod_cast hmant₂_hi
    have hpow : ((10 ^ 19 : ℕ) : ℚ) = (10 : ℚ) ^ (19 : ℕ) := by
      push_cast
      ring
    rw [← hpow]
    exact hcast
  have hlower : (10 : ℚ) ^ (n₁.exponent_ + 18) ≤ n₁.toRat := by
    rw [Number.toRat_of_nonneg n₁ hneg₁]
    have hpow : (10 : ℚ) ^ (n₁.exponent_ + 18) =
        (10 : ℚ) ^ (18 : ℕ) * (10 : ℚ) ^ n₁.exponent_ := by
      rw [← zpow_natCast (10 : ℚ) 18, ← zpow_add₀ (by norm_num : (10 : ℚ) ≠ 0)]
      congr 1
      push_cast
      ring
    rw [hpow]
    exact mul_le_mul_of_nonneg_right hmant₁_lo' (le_of_lt (zpow_pos (by norm_num) _))
  have hupper : n₂.toRat < (10 : ℚ) ^ (n₂.exponent_ + 19) := by
    rw [Number.toRat_of_nonneg n₂ hneg₂]
    have hpow : (10 : ℚ) ^ (n₂.exponent_ + 19) =
        (10 : ℚ) ^ (19 : ℕ) * (10 : ℚ) ^ n₂.exponent_ := by
      rw [← zpow_natCast (10 : ℚ) 19, ← zpow_add₀ (by norm_num : (10 : ℚ) ≠ 0)]
      congr 1
      push_cast
      ring
    rw [hpow]
    exact mul_lt_mul_of_pos_right hmant₂_hi' (zpow_pos (by norm_num) _)
  by_contra hnot
  push_neg at hnot
  have hexp : n₂.exponent_ + 19 ≤ n₁.exponent_ + 18 := by omega
  have hpows : (10 : ℚ) ^ (n₂.exponent_ + 19) ≤
      (10 : ℚ) ^ (n₁.exponent_ + 18) :=
