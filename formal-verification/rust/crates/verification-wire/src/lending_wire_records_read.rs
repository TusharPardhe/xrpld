use super::super::{
    lending_wire_primitive::{Reader, WireError},
    lending_wire_types::*,
};

fn aid(r: &mut Reader) -> Result<AccountId, WireError> {
    Ok(AccountId(r.u64()?))
}
fn oid(r: &mut Reader) -> Result<ObjectId, WireError> {
    Ok(ObjectId(r.u64()?))
}
fn iid(r: &mut Reader) -> Result<IssueId, WireError> {
    Ok(IssueId {
        currency: r.u64()?,
        issuer: aid(r)?,
    })
}
fn vi(r: &mut Reader) -> Result<VaultIdentity, WireError> {
    Ok(VaultIdentity {
        vault_id: oid(r)?,
        owner: aid(r)?,
        account: aid(r)?,
...
fn bi(r: &mut Reader) -> Result<BrokerIdentity, WireError> {
    Ok(BrokerIdentity {
        broker_id: oid(r)?,
        vault_id: oid(r)?,
        owner: aid(r)?,
...
fn li(r: &mut Reader) -> Result<LoanIdentity, WireError> {
    Ok(LoanIdentity {
        loan_id: oid(r)?,
        broker_id: oid(r)?,
        borrower: aid(r)?,
...
fn encode_with<T>(
    x: T,
    write: impl FnOnce(&mut Vec<u8>, T) -> Result<(), WireError>,
...
pub fn encode_broker(x: LoanBroker) -> Result<Vec<u8>, WireError> {
    encode_with(x, super::lending_wire_records_write::broker)
}
pub fn encode_state(x: LendingState) -> Result<Vec<u8>, WireError> {
    encode_with(x, super::lending_wire_records_write::state)
}
pub fn encode_broker_vault(x: BrokerVault) -> Result<Vec<u8>, WireError> {
    encode_with(x, super::lending_wire_records_write::broker_vault)
}
pub fn encode_loan_vault(x: LoanVault) -> Result<Vec<u8>, WireError> {
    encode_with(x, super::lending_wire_records_write::loan_vault)
}
