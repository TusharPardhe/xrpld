...
- Formal repository: `<captured-local-path>`
- Formal Lean source: `<captured-local-path>`
- Bridge: `<captured-local-path>`
- Quaxar: `<captured-local-path>`
...
A fresh `cargo run --quiet` in `<captured-local-path>` reported:
...
An absolute-path LWAB manifest now exists at `<captured-local-path>`. It records all 34 exports, tags 1–27, response tags 129–155, schemas, complete typed results, and the terminal rule: raw transition once, then exactly one Vault association only on a successful result.
...
cargo check --manifest-path <captured-local-path>
# PASS

cargo run --quiet --manifest-path <captured-local-path>
cargo run --release --quiet --manifest-path <captured-local-path>
